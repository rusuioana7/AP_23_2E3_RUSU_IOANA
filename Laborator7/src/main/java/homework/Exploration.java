...
public class Exploration {
    ...
    public void start() {
        synchronized (this) {
            ...
            TimeKeeper timeKeeper = new TimeKeeper(10000, this);
            timeKeeper.start();
        }
    }
    ...
    public void pauseRobot(int seconds, String robotName) {//pause a robot for a specified number of seconds
        synchronized (this) {
            ...
            } else {//if a name isn't specified the exploration is paused for the specified seconds
                running = false;
                try {
                    Thread.sleep(seconds * 1000L);
                } catch (InterruptedException e) {
